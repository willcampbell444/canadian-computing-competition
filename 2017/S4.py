...
class Edge():
	...
	def __lt__(self, other):
		return self.weight < other.weight or (self.weight == other.weight and initit(self))
	def __hash__(self):
...
for l in range(len(inp)):
	...
	if l < num_buildings-1:
		initial.add(Edge(line[0]-1, line[1]-1, line[2]))
	graph.addEdge(int(line[0])-1, int(line[1])-1, int(line[2]))

def initit(x):
	global initial
	return x in initial or Edge(x.edgeTo, x.edgeFrom, x.weight) in initial
